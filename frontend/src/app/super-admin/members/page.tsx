...
export default function SuperAdminMembersPage() {
  ...
  const load = useCallback(() => {
    ...
    fetch(`/api/super-admin/members?${params}`)
      .then((r) => r.json())
      .then((d) => {
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-[var(--color-border)]">
                  ...
                  <th className="text-left py-2 px-3 text-[var(--color-muted)] font-medium">Email</th>
                  ...
                  <th className="text-left py-2 px-3 text-[var(--color-muted)] font-medium">Age</th>
              ...
              <tbody>
                {members.map((m) => (
                  <tr key={m.id} className="border-b border-[var(--color-border)] hover:bg-[var(--color-surface)]">
                    ...
                    <td className="py-2 px-3 text-[var(--color-text)]">{m.email ?? "-"}</td>
                    ...
                    <td className="py-2 px-3 text-[var(--color-text)]">{m.age_group?.replace("_", " ") ?? "-"}</td>
