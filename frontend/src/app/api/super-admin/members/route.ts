...
export async function GET(request: NextRequest) {
  ...
  const [members, countResult, branches] = await Promise.all([
    db.query(
      `SELECT m.id, m.first_name, m.last_name, m.phone, m.email, m.gender, m.age_group,
              m.district, m.occupation, m.marital_status, m.is_first_time, m.created_at,
